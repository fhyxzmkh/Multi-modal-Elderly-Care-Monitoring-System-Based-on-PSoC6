...
char message[] = "{\"radar_transmission\":\"enable\"}";
...
void udp_server_task(void *arg)
{
    ...
    if(pdPASS !=  xTaskCreate(radar_task, "radar_task", configMINIMAL_STACK_SIZE * 8, NULL, 4, &radar_task_handle))
    {
...
#define UDP_SERVER_IP_ADDRESS             MAKE_IPV4_ADDRESS(8,153,193,146)
...
//    xQueueSend( radar_config_queue, ( void * ) &message, ( TickType_t ) 0 );
    while(true)
    {
...
void radar_init()
{
    ...
    if (xSemaphoreTake(sem_udp_payload, portMAX_DELAY) == pdTRUE)
        {
    	strncpy(udp_msg_payload,message,sizeof(message));
