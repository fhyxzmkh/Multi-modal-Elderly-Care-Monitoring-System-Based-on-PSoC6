...
interface PhysiologicalHistoryResponse {
  status: string;
  code: number;
  message: string;
  data: {
    total: number;
    rows: PhysiologicalRecord[];
  };
}

export const PhysiologicalHistoryTable: React.FC = () => {
  const [records, setRecords] = useState<PhysiologicalRecord[]>([]);
  ...
  const fetchHistory = async (page: number) => {
    ...
    try {
      const response =
        await axios_login_instance.get<PhysiologicalHistoryResponse>(
          `/physiological/get/list?page=${page}&pageSize=${pageSize}`,
        );
      ...
      if (response.data.status === "success") {
        ...
        setError(response.data.message || "获取生理数据记录失败");
      }
    } catch (err) {
      console.error("获取生理数据记录失败:", err);
